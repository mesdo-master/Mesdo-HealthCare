    ...
    <div className="relative">
      {toast && (
        <div className="absolute left-1/2 top-0 transform -translate-x-1/2 -translate-y-1/2 z-40">
          <div className="flex items-center gap-3 bg-white border border-green-200 shadow-xl rounded-2xl px-6 py-4 animate-fade-in">
      ...
      <motion.div
        className={`bg-white p-5 ${
          small ? "w-full h-auto" : "w-[1002px] h-[180px] ml-3 min-h-[100px]"
        } rounded-2xl shadow-sm hover:shadow-md transition-all duration-200 border border-[#E4E5E8] relative flex items-center cursor-pointer`}
        onClick={() => navigate(`/jobs/${job._id}`)}
      >
        {/* Logo */}
        <div className="flex flex-col items-center justify-center w-24 h-24 mr-6">
          <img
            alt="Company Logo"
            className="w-[135px] h-[128px] object-contain rounded-lg border border-gray-200 p-1 bg-white"
            src={
              job.hospitalLogo ||
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-gray-500 mb-3 truncate">
            {job.HospitalName} <span className="mx-1">|</span> {job.location}
          </p>
          {/* Tags */}
