...
const AppliedJob = ({ inUserProfile }) => {
  ...
  // Sort applied jobs based on selected option
  const sortedAppliedJobs = useMemo(() => {
    if (!appliedJobs.length) return [];

    const sorted = [...appliedJobs];
    switch (sortBy) {
      case "Recently Applied":
        return sorted.sort((a, b) => {
          const dateA = new Date(a.appliedAt || a.job?.createdAt || 0);
          const dateB = new Date(b.appliedAt || b.job?.createdAt || 0);
          return dateB - dateA;
        });
      case "Job Title":
        return sorted.sort((a, b) => {
          const titleA = (a.job?.jobTitle || "").toLowerCase();
          const titleB = (b.job?.jobTitle || "").toLowerCase();
          return titleA.localeCompare(titleB);
        });
      case "Company Name":
        return sorted.sort((a, b) => {
          const companyA = (a.job?.HospitalName || "").toLowerCase();
          const companyB = (b.job?.HospitalName || "").toLowerCase();
          return companyA.localeCompare(companyB);
        });
      case "Status":
        return sorted.sort((a, b) => {
          const statusA = (a.status || "Applied").toLowerCase();
          const statusB = (b.status || "Applied").toLowerCase();
                    ...
                    <AnimatePresence mode="popLayout">
                      <div className="space-y-4">
                        {sortedAppliedJobs.map((application, index) => {
                          ...
                          const appliedAt =
                            application.appliedAt || job.createdAt;
                          const currentStatusIndex =
                            getStatusIndex(applicationStatus);
