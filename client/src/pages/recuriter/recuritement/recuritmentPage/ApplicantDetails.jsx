...
const schoolIcon = "https://res.cloudinary.com/dy9voteoc/image/upload/v1744904300/famicons_school-outline_kuqz1z.png";
const schoolIcon2 = "https://res.cloudinary.com/dy9voteoc/image/upload/v1744904308/School_2_bzrqw7.png";
const schoolIcon3 = "https://res.cloudinary.com/dy9voteoc/image/upload/v1744904312/School_3_egvf9b.png";
...
function TopBar({ setSelectedApplicant }) {
    ...
    return (
        <div className="flex items-center justify-between px-4 py-2 border-b border-gray-200 bg-white mt-[7vh]">
            ...
            <button className="p-1 text-gray-600 hover:text-gray-900" onClick={() => setSelectedApplicant(null)}>
...
function ProfileHeaderWithTabs({ activeTab, onTabClick, applicant }) {
    return (
        <div className="bg-white shadow-sm">
            ...
            <div className="flex items-center justify-between px-4 py-4 h-38">
                <div className="flex items-center space-x-4">
                    <img
                        src={applicant.profilePicture || "https://res.cloudinary.com/dy9voteoc/image/upload/v1743420262/default-avatar-profile-icon-social-media-user-image-gray-avatar-icon-blank-profile-silhouette-vector-illustration_561158-3383_sxcncq.avif"}
                        alt="Profile"
...
function TabItem({ label, active, onClick }) {
    return (
        <button
            onClick={onClick}
            className={`relative py-3 text-sm font-medium text-gray-600 hover:text-gray-800 transition ${active ? "text-gray-800" : ""
                }`}
        >
            {label}
            {active && (
...
function ProfileHeader({ activeTab, onTabClick, applicant, setSelectedApplicant }) {
    console.log(applicant);
    return (
        <div>
            <TopBar setSelectedApplicant={setSelectedApplicant} />
            <ProfileHeaderWithTabs activeTab={activeTab} onTabClick={onTabClick} applicant={applicant} />
            <div className="mt-6">
    ...
    return (
        <div className="bg-white rounded-md shadow-sm p-4 md:p-6">
            ...
            <div
                ...
                dangerouslySetInnerHTML={{ __html: applicant?.about || "<em>No information provided.</em>" }}
            />
        </div>
    );
}
    ...
    return (
        <div className="bg-white rounded-md shadow-sm p-4 md:p-6">
            ...
            {experiences.length > 0 ? (
                ...
                <div className="text-sm text-gray-500 italic">No work experience added yet.</div>
            )}
        </div>
    );
}

function WorkExperienceItem({ icon, title, organization, date, htmlDescription }) {
    return (
        <div className="flex items-start space-x-3">
            {/* Icon on the left */}
            <div className="mt-1">{icon}</div>
        ...
        <div className="bg-white rounded-md shadow-sm p-4 md:p-6 space-y-8">
            ...
            <div>
                ...
                {skills.length > 0 ? (
                    ...
                    <p className="text-sm text-gray-500 mt-2 italic">No skills added yet.</p>
                )}
            </div>
            ...
            <div>
                ...
                {certifications.length > 0 ? (
                    ...
                    <p className="text-sm text-gray-500 mt-2 italic">No certificates or awards added yet.</p>
                )}
            </div>
        </div>
    );
    ...
    return (
        <div className="flex items-start space-x-3">
            ...
            <div className="text-sm text-gray-600">
                ...
                <p className="mt-1">{issuer} | {date}</p>
            </div>
        </div>
    );
}
    ...
    return (
        <div className="bg-white rounded-md shadow-sm p-4">
            ...
            <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-gray-700">{applicant.location.city}, {applicant.location.state}</span>
            </div>
        </div>
    );
}
            ...
            <div className="text-left mt-4">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                    {skills.map((skill) => (
                        <span
                            key={skill.name}
                            className={`inline-block text-sm py-1 px-3 rounded-full ${skill.matched
            ...
            <div className="text-left mt-4">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                    {languages.map((lang) => (
                        <span
                            key={lang.name}
                            className={`inline-block text-sm py-1 px-3 rounded-full ${lang.matched
                                ? "bg-blue-500 text-white"
                                : "bg-red-100 text-red-600"
                                }`}
                        >
            ...
            <div className="text-left mt-4">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                    {specializations.map((spec) => (
                        <span
                            key={spec.name}
                            className={`inline-block text-sm py-1 px-3 rounded-full ${spec.matched
                                ? "bg-blue-500 text-white"
                                : "bg-red-100 text-red-600"
                                }`}
                        >
...
export function ApplicantDetails({ applicant, setSelectedApplicant }) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <ProfileHeader activeTab={activeTab} onTabClick={handleTabClick} applicant={applicant} setSelectedApplicant={setSelectedApplicant} />
            </div>
        </div>
    );
}