...
const acceptRequest = async (req, res) => {
  try {
    ...
    // ✅ Update the notification status instead of deleting it
    ...
    // ✅ Also emit to recipient for real-time UI updates
    io.to(recipientId.toString()).emit("followRequestAccepted", {
      notificationId,
      senderId,
      recipientId,
    });
...
const rejectRequest = async (req, res) => {
  try {
    ...
    // ✅ Update the notification status instead of deleting it
    notificationFound.status = "rejected";
    await notificationFound.save();

    // ✅ Emit socket event for real-time UI updates
    io.to(recipientId.toString()).emit("followRequestRejected", {
      notificationId,
      senderId,
      recipientId,
    });
...
const followOrganization = async (req, res) => {
  ...
  try {
    ...
    io.to(organization.userId._id.toString()).emit(
      "newNotification",
      notification
    );
    ...
    return res
      .status(200)
      .json({ message: "Followed organization and notification sent" });
  } catch (err) {
...
const unfollowOrganization = async (req, res) => {
  ...
  try {
    ...
    if (!user.followedOrganizations.includes(orgId)) {
      return res
        .status(400)
        .json({ message: "You are not following this organization" });
    }
    ...
    return res
      .status(200)
      .json({ message: "Unfollowed organization successfully" });
  } catch (err) {
